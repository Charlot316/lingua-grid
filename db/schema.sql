...
CREATE TABLE concepts (
    ...
    gloss_zh TEXT NOT NULL,        -- 中文简述，如 "太阳 (天体)", "天/日 (时间)"
    category TEXT,                 -- 分类: Nature, Time, Emotion
...
CREATE TABLE words (
    id TEXT PRIMARY KEY,           -- UUID
    language TEXT NOT NULL,        -- 'en', 'jp', 'kr', 'th', 'fr'
    text TEXT NOT NULL,            -- 单词拼写: "Sun", "日", "Excited"
    pronunciation TEXT,            -- 音标/罗马音: "/sʌn/", "Nichi", "Tawan"
    audio_url TEXT,                -- 发音文件路径
    difficulty_level INTEGER,      -- 1 (基础) - 5 (高阶/GRE) - 10 (母语级罕见)
    
    -- 词源/汉字辅助字段 (这是你的强项区)
    hanja_script TEXT,             -- 仅用于韩/日: 对应的汉字，如 "太阳", "気持"
    etymology_root TEXT,           -- 词根: "sol-", "helio-"
...
CREATE TABLE senses (
    ...
    is_primary BOOLEAN DEFAULT FALSE, -- 在这个概念下，这是不是最常用的那个词？
...
CREATE TABLE word_alignments (
    ...
    alignment_type TEXT CHECK(alignment_type IN ('EXACT', 'COGNATE', 'APPROXIMATE')),
    
    note TEXT, -- 备注差异，如 "Tone rule differs"
...
CREATE TABLE sentences (
    ...
    translation_zh TEXT,
    highlight_indices JSON, -- 存储高亮位置，例如 [0, 3]
